...
- 料金は制度上の**目安**表記。最終確認は市区町村の窓口・主治医・各保険者へ
- お問い合わせ／応募フォームは送信時にメールソフトを起動します（サーバー送信の接続も可能）
- **先輩の声**（`recruitVoices`）・**1日の流れ**（`recruitDayFlow`）は例／サンプル
- 求人の構造化データ（JobPosting）の `datePosted` は `prerender.js` の定数。
  募集を再開したときのみ更新し、**募集終了時はスキーマ削除か `validThrough` を過去日に**

### 📈 採用効果を最大化するために事業者から回収したい実データ（リサーチ根拠つき）

応募率に最も効くと調査で裏づけられた開示情報です。入手でき次第サイトに反映推奨:

1. **給与レンジ・モデル年収**（基本給＋手当の内訳、賞与実績）→ ページ表示＋JobPosting `baseSalary` 追加
2. **オンコールの実態**（待機回数/月・実出動頻度・手当額・免除条件）→ 訪問看護転職の最大障壁(44.7%)
3. **教育体制の実数**（同行期間の目安・未経験入職者の割合）
4. **働き方の実数**（年間休日・有給取得実績・平均残業・直行直帰可否・移動手段）
5. **スタッフ実データ＋実写真**（人数・平均年齢・子育て中比率、事務所/カンファ風景）

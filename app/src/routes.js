// Single source of truth for routes + navigation.
// Pages are split per nav item; Header/Footer/sitemap all read from here.
import { articles } from "./articles.js";

export const navLinks = [
  { label: "サービス内容", to: "/service" },
  { label: "ご利用の流れ", to: "/flow" },
  { label: "よくある質問", to: "/faq" },
  { label: "コラム", to: "/column" },
  { label: "事業所概要", to: "/company" },
  { label: "お問い合わせ", to: "/contact" },
  { label: "看護師採用", to: "/recruit", accent: true },
];

// Per-page <title> / description used by useDocumentMeta.
export const SITE_NAME = "訪問看護ステーションSHARK";

export const pageMeta = {
  home: {
    title: `${SITE_NAME}｜奈良県（橿原市・大和高田市）の精神科訪問看護`,
    description:
      "奈良県全域対応・精神科特化の訪問看護ステーションSHARK（橿原市・大和高田市中心）。24時間365日、服薬管理・ご家族支援・主治医連携までサポート。医療保険・自立支援医療制度対応、相談無料。",
  },
  service: {
    title: `サービス内容・料金｜精神科訪問看護のSHARK（奈良）`,
    description:
      "精神科訪問看護のサービス内容（傾聴・服薬管理・ご家族支援・主治医連携など8項目）と対応疾患、医療保険・自立支援医療制度による料金の目安をご紹介。奈良県全域に訪問対応します。",
  },
  flow: {
    title: `ご利用の流れ・対応エリア（奈良県全域）｜${SITE_NAME}`,
    description:
      "精神科訪問看護のご利用開始までの7つのステップと、奈良県内の訪問対応エリア（橿原市・大和高田市・奈良市ほか）をご案内します。指示書の手続きもサポートします。",
  },
  faq: {
    title: `よくある質問｜精神科訪問看護のSHARK（奈良）`,
    description:
      "精神科訪問看護の対象・費用・自立支援医療制度・訪問頻度・24時間対応・家族相談など、よくある17の質問にお答えします。",
  },
  company: {
    title: `事業所概要｜${SITE_NAME}`,
    description: "訪問看護ステーションSHARK（株式会社B-Nuts）の事業所概要です。",
  },
  recruit: {
    title: `看護師求人・採用情報（常勤/非常勤）｜${SITE_NAME}`,
    description:
      "奈良県（橿原市・大和高田市中心）の精神科訪問看護ステーションSHARKの看護師求人。訪問看護未経験・ブランク・子育て中の方も歓迎。常勤/非常勤、見学・カジュアル面談OK。LINEで気軽に相談できます。",
  },
  contact: {
    title: `お問い合わせ｜${SITE_NAME}`,
    description:
      "ご本人・ご家族・医療機関・施設関係者様からのご相談を受け付けています。電話・LINE・メールでお気軽にお問い合わせください。",
  },
  column: {
    title: `コラム・お役立ち情報｜${SITE_NAME}`,
    description:
      "精神科訪問看護とは？自立支援医療制度の使い方は？費用の目安は？——奈良県の精神科特化訪問看護ステーションが、知っておくと安心な情報をわかりやすく解説します。",
  },
  notFound: {
    title: `ページが見つかりません｜${SITE_NAME}`,
    description: "",
    robots: "noindex, follow",
  },
};

// Production origin (update if a custom domain is connected).
export const SITE_URL = "https://shark-nara.jp";

// Path → meta, used by routing, prerendering and the sitemap.
export const ROUTE_META = {
  "/": pageMeta.home,
  "/service": pageMeta.service,
  "/flow": pageMeta.flow,
  "/faq": pageMeta.faq,
  "/company": pageMeta.company,
  "/recruit": pageMeta.recruit,
  "/contact": pageMeta.contact,
  "/column": pageMeta.column,
  ...Object.fromEntries(
    articles.map((a) => [
      `/column/${a.slug}`,
      { title: `${a.title}｜${SITE_NAME}`, description: a.description, article: a },
    ])
  ),
};

export const PRERENDER_PATHS = Object.keys(ROUTE_META);
